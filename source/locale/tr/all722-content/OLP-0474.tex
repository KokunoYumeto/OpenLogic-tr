% Generated from locale/tr/content/normal-modal-logic/sequent-calculus/more-rules.tex; do not hand-edit.


\olfileid[tr]{nml}{seq}{mru}
...
\olref{tab:more-rules}'deki ek kuralları inceleriz.
...
Bu kurallar eklendiğinde \olref{tab:logics-rules}'de verilen mantıklar
için sağlam ve tam sistemler elde edilir.

\begin{table}
  \begin{center}
    \begin{tabular}{lll}
      \hline
      Mantık & $R$ bağıntısı \dots{} & Kurallar\\
      \hline
      $\Log{T} = \Log{KT}$ & yansımalı & $\Box$,
      \iftag{prvBox}{T$\Box$}{}%
      \iftag{notprvBox,notprvDiamond}{}{, }%
      \iftag{prvDiamond}{T$\Diamond$}{}
      \\ \hline
      $\Log{D} = \Log{KD}$ & seri & $\Box$,
      \iftag{prvBox}{%
    ...
    \end{tabular}
  \end{center}
  \caption{Çeşitli modal mantıklar için sekant kuralları.}
  \ollabel{tab:logics-rules}
\end{table}

\iftag{prvBox}{
\begin{ex}
  $\Log{K4} \Proves \Ax{4}$, yani $\Box!A \lif \Box\Box!A$ olduğunu
  gösteren bir sekant !!{derivation}{}i verelim.
  \begin{prooftree}
...
\begin{ex}
  $\Log{K4} \Proves \Ax{4}$, yani $\Diamond\Diamond!A \lif \Diamond!A$
  olduğunu gösteren bir sekant !!{derivation}{}i verelim.
  \begin{prooftree}
...
\begin{ex}
\Log{S5} sekant hesabı \Cut{} kuralı olmadan tam değildir; örneğin
\iftag{prvBox}{$\Diamond\Box !A \lif !A$}{$!A \lif \Box\Diamond !A$}
$\Log{S5}$'te geçerli olduğu hâlde \Cut{} olmadan bir ispatı yoktur.
İşte \Cut{} kullanan bir !!a{derivation}:
\begin{prooftree}
...
\begin{prob}
Aşağıdakileri gösteren sekant !!{derivation}s{}i verin:
  \begin{enumerate}
