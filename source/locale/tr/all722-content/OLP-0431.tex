% Generated from locale/tr/content/normal-modal-logic/axioms-systems/proofs-in-K.tex; do not hand-edit.


\olfileid[tr]{nml}{prf}{prk}

\olsection{\Log{K}'de İspatlar}

En küçük modal sistemde ispat alıştırması yapmak amacıyla,
\olref[syn][sch]{tab:valid-invalidSchemas}'in sol tarafındaki geçerli
!!{formula}s için birer $\Log{K}$-ispatı verilebildiğini gösteriyoruz.
...
\begin{proof}
  ...
  \end{derivation}
  6. ve 9. satırlardaki !!{formula}s sırasıyla şu totolojilerin örnekleridir:
  \begin{align*}
...
\begin{proof}
  ...
  \end{derivation}
  8. ve 10. satırlardaki !!{formula}s şu totolojilerin örnekleridir:
  \begin{align*}
  ...
  \end{derivation}
  Son satır $\lnot \Box p \lif \Diamond\lnot p$'dir; çünkü $\Diamond$,
  $\lnot\Box\lnot$ olarak tanımlanır. 5. satırdaki !!{formula},
  \[
    (p \lif q) \lif (\lnot q \lif \lnot p).
  \]
  totolojisinin bir örneğidir.
...
\begin{prob}
  $\Log{K}$ içinde !!{derivation}s bulun; bunlar aşağıdaki !!{formula}s
  için olsun:
  \begin{enumerate}
